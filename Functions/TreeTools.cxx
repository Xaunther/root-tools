...
TChain* GetChain(string filedir, string tuplename, bool verbose)
{
  ...
  if (verbose) {cout << "Reading Tree " << treename << endl;}
...
int GetCorrEvents(TTree* alltree1, TTree* alltree2, string cuts1, string cuts2)
{
  ...
  {
    tree1->GetEntry(i);
    if (i == 0) {currentevt = evtnumber1; id_evtnumber1[N1] = evtnumber1; N1++;}
  ...
  {
    tree2->GetEntry(i);
    if (i == 0) {currentevt = evtnumber2; id_evtnumber2[N2] = evtnumber2; N2++;}
...
bool TreeExists(string filedir, string tuplename)
{
  //Get treename
  string treename;
  if (tuplename != "") {treename = tuplename;}
  else {treename = GetTreeName(filedir);}
